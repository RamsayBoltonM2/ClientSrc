...
void CPythonTextTail::RegisterCharacterTextTail(DWORD dwGuildID, BYTE dwNewIsGuildName, DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight)
{
	...
	if (0 != dwGuildID)
	{
		...
		if (dwNewIsGuildName == 3)
			strGuildName.insert(0, "|cffADFF2F[Lider] |cffffcc00 ");
		else if (dwNewIsGuildName == 2)
			strGuildName.insert(0, "|cffADFF2F[General] |cffffcc00 ");
...
void CPythonTextTail::RegisterItemTextTail(DWORD VirtualID, DWORD bk, const char * c_szText, CGraphicObjectInstance * pOwner)
{
#ifdef __DEBUG
	char szName[256];
	...
	if (!CPythonSkill::Instance().GetSkillData(bk, &c_pSkillData) || bk == 0)
		sprintf(szName, "%s", c_szText);
	else
	{
		sprintf(szName, "%s %s",c_pSkillData->GradeData[0].strName.c_str(), c_szText);
	}

	TTextTail * pTextTail = RegisterTextTail(VirtualID, szName, pOwner, c_TextTail_Name_Position, c_TextTail_Item_Color);
	m_ItemTextTailMap.insert(TTextTailMap::value_type(VirtualID, pTextTail));
#endif
}
