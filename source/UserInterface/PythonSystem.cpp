...
void CPythonSystem::SetDefaultConfig()
{
	...
	m_Config.bShowRuzgar = true;
	m_Config.bShowKritik = true;
	m_Config.bShowTest = true;
	m_Config.bShowDuello = true;
	m_Config.bShowMor = true;
#ifdef ENABLE_SHOP_BOARD_RANGE
	m_Config.shopnames_range = 0.0f;
#endif
	m_Config.bShowClock = true;
...
bool CPythonSystem::IsShowRuzgar()
{
	return m_Config.bShowRuzgar;
}

void CPythonSystem::SetShowRuzgar(int iFlag)
{
	m_Config.bShowRuzgar = iFlag == 1 ? true : false;
}

bool CPythonSystem::IsShowKritik()
{
	return m_Config.bShowKritik;
}

void CPythonSystem::SetShowKritik(int iFlag)
{
	m_Config.bShowKritik = iFlag == 1 ? true : false;
}


bool CPythonSystem::IsShowTest()
{
	return m_Config.bShowTest;
}

void CPythonSystem::SetShowTest(int iFlag)
{
	m_Config.bShowTest = iFlag == 1 ? true : false;
}

bool CPythonSystem::IsShowDuello()
{
	return m_Config.bShowDuello;
}

void CPythonSystem::SetShowDuello(int iFlag)
{
	m_Config.bShowDuello = iFlag == 1 ? true : false;
}

bool CPythonSystem::IsShowMor()
{
	return m_Config.bShowMor;
}

void CPythonSystem::SetShowMor(int iFlag)
{
	m_Config.bShowMor = iFlag == 1 ? true : false;
}

bool CPythonSystem::IsShowClock()
{
	return m_Config.bShowClock;
}


void CPythonSystem::SetShowClock(int iFlag)
{
	m_Config.bShowClock = iFlag == 1 ? true : false;
}
...
bool CPythonSystem::LoadConfig()
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		else if (!stricmp(command, "SHOW_Ruzgar"))
			m_Config.bShowRuzgar = atoi(value) == 1 ? true : false;
		else if (!stricmp(command, "SHOW_MOR"))
			m_Config.bShowMor = atoi(value) == 1 ? true : false;
		else if (!stricmp(command, "SHOW_KRITIK"))
			m_Config.bShowKritik = atoi(value) == 1 ? true : false;
		else if (!stricmp(command, "SHOW_TEST"))
			m_Config.bShowTest = atoi(value) == 1 ? true : false;
		else if (!stricmp(command, "SHOW_DUELLO"))
			m_Config.bShowDuello = atoi(value) == 1 ? true : false;
		else if (!stricmp(command, "SHOW_CLOCK"))
			m_Config.bShowClock = atoi(value) == 1 ? true : false;
...
bool CPythonSystem::SaveConfig()
{
	...
	fprintf(fp, "SHOW_Ruzgar		%d\n", m_Config.bShowRuzgar);
	fprintf(fp, "SHOW_MOR		%d\n", m_Config.bShowMor);
	fprintf(fp, "SHOW_KRITIK		%d\n", m_Config.bShowKritik);
	fprintf(fp, "SHOW_TEST		%d\n", m_Config.bShowTest);
	fprintf(fp, "SHOW_DUELLO		%d\n", m_Config.bShowDuello);
	fprintf(fp, "SHOP_CLOCK			%d\n", m_Config.bShowClock);
	fprintf(fp, "\n");
