...
BOOL CInstanceBase::IsSameEmpire(CInstanceBase& rkInstDst)
{
	...
	if (IsGameMaster())
		return TRUE;//if you got bugs don`t add this line.
...
bool CInstanceBase::Create(const SCreateData& c_rkCreateData)
{
	...
	int i=0;
	if(c_rkCreateData.m_dwRace == 34065 || c_rkCreateData.m_dwRace == 34066)
	{
	...
	if (IsPC())
	{
		...
		SetWeapon(c_rkCreateData.m_dwWeapon, c_rkCreateData.m_dwWeaponEvolution);
	}
...
#if defined(ENABLE_GROWTH_PET_SYSTEM) || defined(ENABLE_SUPPORT_SHAMAN_SYSTEM)
...
void CInstanceBase::SetArmor(DWORD dwArmor)
{
	...
	if (__ArmorVnumToShape(dwArmor, &dwShape))
	{
		...
		if (CItemManager::Instance().GetItemDataPointer(dwArmor, &pItemData))
		{
			...
			if (pItemData->GetIndex() == 41330 || pItemData->GetIndex() == 41331)
...
void CInstanceBase::SetAcce(DWORD dwSash)
{
	if (!IsPC())
		return;
	...
	fSpecular /= 100.0f;
	m_awPart[CRaceData::PART_ACCE] = dwSash;
...
bool CInstanceBase::SetWeapon(DWORD eWeapon, DWORD dwEvolution)
{
	if (IsPoly())
		return false;
