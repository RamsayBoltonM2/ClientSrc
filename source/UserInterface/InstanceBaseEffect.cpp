...
//#ifdef ENABLE_VIP_AFFECTIVE_SYSTEM
...
void CInstanceBase::__AttachSelectEffectStone()
{
	__EffectContainer_AttachEffect(EFFECT_STONE);
}
void CInstanceBase::__DetachSelectEffectStone()
{
	__EffectContainer_DetachEffect(EFFECT_STONE);
}
void CInstanceBase::__AttachTargetEffectStone()
{
	__EffectContainer_AttachEffect(EFFECT_TARGET_STONE);
}
void CInstanceBase::__DetachTargetEffectStone()
{
	__EffectContainer_DetachEffect(EFFECT_TARGET_STONE);
}
...
#ifdef ENABLE_MELEY_LAIR_DUNGEON//maybe_fixed_bug 10.12.2016
...
void CInstanceBase::UpdateTextTailLevel(DWORD level)
{
	...
	char szText[256];
	if (IsGameMaster())
		sprintf(szText, "Lv %d ", level); // Playeri
	else if (IsNPC() && (GetRace() >= 30000 && GetRace() <= 30004))
		sprintf(szText, "Lv %d ", level); // Playeri
	else if (IsEnemy())
