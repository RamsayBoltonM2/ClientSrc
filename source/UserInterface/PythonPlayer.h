...
class CPythonPlayer : public CSingleton<CPythonPlayer>, public IAbstractPlayer
{
	...
	public:
		...
		void	SetCurrentOfflineShopBar1(DWORD dwBar);
		DWORD	GetCurrentOfflineShopBar1();
		void	SetCurrentOfflineShopBar2(DWORD dwBar);
		DWORD	GetCurrentOfflineShopBar2();
		void	SetCurrentOfflineShopBar3(DWORD dwBar);
		DWORD	GetCurrentOfflineShopBar3();
		void	SetCurrentOfflineShopSoulStone(DWORD dwSoulStone);
		DWORD	GetCurrentOfflineShopSoulStone();
		void	SetCurrentOfflineShopDragonScale(DWORD dwDragonScale);
		DWORD	GetCurrentOfflineShopDragonScale();
		void	SetCurrentOfflineShopDragonClaw(DWORD dwDragonClaw);
		DWORD	GetCurrentOfflineShopDragonClaw();
		...
		bool	ApplicationIsMinimized(DWORD dwMessageIndex);
		bool	IsUsingKeyToCollect();
#endif
		void	SetAffect(UINT uAffect);
		void	ResetAffect(UINT uAffect);
#ifdef ENABLE_SLOTING_EX_SYSTEM
		int		GetNewQuickPage();
		void	SetNewQuickPage(int nPageIndex);
		void	AddNewQuickSlot(int QuickslotIndex, char IconType, char IconPosition);
		void	DeleteNewQuickSlot(int QuickslotIndex);
		void	MoveNewQuickSlot(int Source, int Target);
		
		void	RequestAddLocalNewQuickSlot(DWORD dwLocalSlotIndex, DWORD dwWndType, DWORD dwWndItemPos);
		void	RequestAddToEmptyLocalNewQuickSlot(DWORD dwWndType, DWORD dwWndItemPos);
		void	RequestMoveGlobalNewQuickSlotToLocalQuickSlot(DWORD dwGlobalSrcSlotIndex, DWORD dwLocalDstSlotIndex);
		void	RequestDeleteGlobalNewQuickSlot(DWORD dwGlobalSlotIndex);
		void	RequestUseLocalNewQuickSlot(DWORD dwLocalSlotIndex);
		DWORD	LocalNewQuickSlotIndexToGlobalQuickSlotIndex(DWORD dwLocalSlotIndex);

		void	GetGlobalNewQuickSlotData(DWORD dwGlobalSlotIndex, DWORD* pdwWndType, DWORD* pdwWndItemPos);
		void	GetLocalNewQuickSlotData(DWORD dwSlotPos, DWORD* pdwWndType, DWORD* pdwWndItemPos);
		void	RemoveNewQuickSlotByValue(int iType, int iPosition);
#endif
		void	ClearAffects();
...
#ifdef ENABLE_SLOTING_EX_SYSTEM
		TNewQuickSlot &	__RefLocalNewQuickSlot(int SlotIndex);
		TNewQuickSlot &	__RefGlobalNewQuickSlot(int SlotIndex);
#endif
