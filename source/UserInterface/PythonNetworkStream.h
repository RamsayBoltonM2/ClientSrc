...
class CPythonNetworkStream : public CNetworkStream, public CSingleton<CPythonNetworkStream>
{
	public:
		...
		enum
		{
			...
			ACCOUNT_CHARACTER_SLOT_WARPOINT,
		};
	...
	public:
		...
		bool SendNewQuickSlotAddPacket(BYTE wpos, BYTE type, BYTE pos);
		bool SendNewQuickSlotDelPacket(BYTE wpos);
		bool SendNewQuickSlotMovePacket(BYTE wpos, BYTE change_pos);
#endif
		...
		bool SendShopSearchBuyPacket(int iVid, BYTE bPos, int iBuyType);
		bool SendShopSearchFindPacket(int iJob, int iType, int iSubType, int iMinLevel, int iMaxLevel, int iMinRefine, int iMaxRefine, int iMinPrice, int iMaxPrice, int iMinCheque, int iMaxCheque, const char* pcItemName);
		...
		bool SendAddOfflineShopItem(TItemPos bDisplayPos, BYTE bPos, int lPrice, int iPrice, short sPriceType);
		...
		bool SendOfflineShopWithdrawBar1(DWORD dwBar);
		bool SendOfflineShopWithdrawBar2(DWORD dwBar);
		bool SendOfflineShopWithdrawBar3(DWORD dwBar);
		bool SendOfflineShopWithdrawSoulStone(DWORD dwSoulStone);
		bool SendOfflineShopWithdrawDragonScale(DWORD dwDragonScale);
		bool SendOfflineShopWithdrawDragonClaw(DWORD dwDragonClaw);
...
#ifdef ENABLE_GF_CHEQUE_SYSTEM
		bool SendExchangeChequeAddPacket(BYTE cheque);
#endif
		...
		// Private Shop
		bool SendBuildPrivateShopPacket(const char * c_szName, const std::vector<TShopItemTable2> & c_rSellingItemStock);
...
#endif
#ifdef ENABLE_INGAME_SUPPORT_SYSTEM
		bool RecvTicketInfoLogs();
		bool RecvTicketInfoLogsReply();
#endif
...
#ifdef ENABLE_SLOTING_EX_SYSTEM
		bool RecvNewQuickSlotAddPacket();
		bool RecvNewQuickSlotDelPacket();
		bool RecvNewQuickSlotMovePacket();
#endif
