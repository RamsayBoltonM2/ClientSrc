...
bool CPythonApplication::Process()
{
	...
	typedef DWORD(__cdecl * t_getTime)();
	static t_getTime gTime1 = (t_getTime)GetProcAddress(GetModuleHandleA("KERNELBASE.dll"), "GetTickCount");
	static DWORD s_time2 = 0;
	static DWORD s_time1 = 0;
	static bool s_tmc = false;
	if (!s_tmc)
	{
		if (!GetModuleHandleA("KERNELBASE.dll") || !GetProcAddress(GetModuleHandleA("KERNELBASE.dll"), "GetTickCount"))
		{
			DWORD dwA = (DWORD)GetProcAddress(GetModuleHandleA("kernel32.dll"), "GetTickCount");
			DWORD dwNewL = (DWORD)malloc(0x20);
			DWORD dwO = 0;
			VirtualProtect((void*)dwNewL, 0x20, PAGE_EXECUTE_READWRITE, &dwO);
			memcpy((void*)dwNewL, (void*)dwA, 0x20);
			gTime1 = (t_getTime)dwNewL;
		}
		s_time2 = timeGetTime();
		s_time1 = gTime1();
		s_tmc = true;
	}
	if (gTime1() - s_time1 > 2000)
	{
		int f1 = timeGetTime() - s_time2;
		int f2 = gTime1() - s_time1;
		if (f1 / f2 > 2)
		{
			Destroy();
		}
