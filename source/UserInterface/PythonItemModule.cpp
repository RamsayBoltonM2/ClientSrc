...
PyObject * itemCanAddToNewQuickSlotItem(PyObject * poSelf, PyObject * poArgs)
{
	int iItemIndex;
...
PyObject * itemIsCostumeScroll(PyObject * poSelf, PyObject * poArgs)
{
	int iItemIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iItemIndex))
		return Py_BadArgument();
	...
	int iSubType = pItemData->GetSubType();
	if (iType == CItemData::ITEM_TYPE_USE)
	if (iSubType == CItemData::USE_ADD_ATTRIBUTE_COSTUME || iSubType == CItemData::USE_CHANGE_ATTRIBUTE_COSTUME)
	{
		return Py_BuildValue("i", TRUE);
	}
...
void initItem()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "IsCostumeScroll", 				itemIsCostumeScroll, 					METH_VARARGS },
		{ "IsKey",							itemIsKey,								METH_VARARGS },
		{ "IsMetin",						itemIsMetin,							METH_VARARGS },
#ifdef ENABLE_SLOTING_EX_SYSTEM
		{ "CanAddToNewQuickSlotItem",			itemCanAddToNewQuickSlotItem,				METH_VARARGS },
#endif
