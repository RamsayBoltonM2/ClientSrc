...
void SNetworkActorData::__copy__(const SNetworkActorData& src)
{
	...
	m_dwWeaponEvolution = src.m_dwWeaponEvolution;
...
CInstanceBase* CNetworkActorManager::__AppendCharacterManagerActor(SNetworkActorData& rkNetActorData)
{
	...
	kCreateData.m_dwWeaponEvolution=rkNetActorData.m_dwWeaponEvolution;
	...
	if (pNewInstance->GetRace() >= 30000 && pNewInstance->GetRace() <= 30004)
	{
		...
		{
			char szName[512];
			sprintf(szName, "%s", pkInstMain->GetNameString());
...
void CNetworkActorManager::UpdateActor(const SNetworkUpdateActorData& c_rkNetUpdateActorData)
{
	...
	if (pkInstFind)
	{
		...
		pkInstFind->ChangeWeapon(c_rkNetUpdateActorData.m_dwWeapon, c_rkNetUpdateActorData.m_dwWeaponEvolution);
		...
		pkInstFind->SetWeaponEvolution(c_rkNetUpdateActorData.m_dwWeaponEvolution);
	...
	rkNetActorData.m_dwWeaponEvolution=c_rkNetUpdateActorData.m_dwWeaponEvolution;
